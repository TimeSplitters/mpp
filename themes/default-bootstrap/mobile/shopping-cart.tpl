...
{else}
	...
	<div id="order-detail-content" class="table_block table-responsive">
		<div id="cart_summary" class="table table-bordered {if $PS_STOCK_MANAGEMENT}stock-management-on{else}stock-management-off{/if}">
			<div>
				...
				<div>
					...
					{/if}
						<div class="cart_discount {if $discount@last}last_item{elseif $discount@first}first_item{else}item{/if}" id="cart_discount_{$discount.id_discount}">
							<div class="cart_discount_name" colspan="{if $PS_STOCK_MANAGEMENT}3{else}2{/if}">{$discount.name}</div>
							<div class="cart_discount_price">
								<span class="price-discount">
								{if !$priceDisplay}{displayPrice price=$discount.value_real*-1}{else}{displayPrice price=$discount.value_tax_exc*-1}{/if}
								</span>
							</div>
							<div class="cart_discount_delete">1</div>
            ...
            {if $use_taxes}
                {if $priceDisplay}
					<div class="cart_total_price">
						<div rowspan="{$rowspan_total}" colspan="3" id="cart_voucher" class="cart_voucher">
                            {if $voucherAllowed}
						...
						<div colspan="{$col_span_subtotal}" class="text-right">{if $display_tax_label}{l s='Total products (tax excl.)'}{else}{l s='Total products'}{/if}</div>
						<div colspan="2" class="price" id="total_product">{displayPrice price=$total_products}</div>
					</div>
                {else}
					<div class="cart_total_price">
						<div rowspan="{$rowspan_total}" colspan="2" id="cart_voucher" class="cart_voucher">
                            {if $voucherAllowed}
								<form action="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" id="voucher">
									<fieldset>
										<h4>{l s='Vouchers'}</h4>
						...
						<div colspan="{$col_span_subtotal}" class="text-right">{if $display_tax_label}{l s='Total products (tax incl.)'}{else}{l s='Total products'}{/if}</div>
						<div colspan="2" class="price" id="total_product">{displayPrice price=$total_products_wt}</div>
					</div>
                {/if}
            {else}
				<div class="cart_total_price">
					...
					<div colspan="{$col_span_subtotal}" class="text-right">{l s='Total products'}</div>
					<div colspan="2" class="price" id="total_product">{displayPrice price=$total_products}</div>
				</div>
            {/if}
			<div{if $total_wrapping == 0} style="display: none;"{/if}>
				...
				<div colspan="2" class="price-discount price" id="total_wrapping">
                    {if $use_taxes}
                        {if $priceDisplay}
                ...
                {if $use_taxes && $total_shipping_tax_exc != $total_shipping}
                    {if $priceDisplay}
						<div class="cart_total_delivery{if $total_shipping_tax_exc <= 0} unvisible{/if}">
							<div colspan="{$col_span_subtotal}" class="text-right">{if $display_tax_label}{l s='Total shipping (tax excl.)'}{else}{l s='Total shipping'}{/if}</div>
							<div colspan="2" class="price" id="total_shipping">{displayPrice price=$total_shipping_tax_exc}</div>
						</div>
                    {else}
						<div class="cart_total_delivery{if $total_shipping <= 0} unvisible{/if}">
							<div colspan="{$col_span_subtotal}" class="text-right">{if $display_tax_label}{l s='Total shipping (tax incl.)'}{else}{l s='Total shipping'}{/if}</div>
							<div colspan="2" class="price" id="total_shipping" >{displayPrice price=$total_shipping}</div>
						</div>
                    {/if}
                {else}
					<div class="cart_total_delivery{if $total_shipping_tax_exc <= 0} unvisible{/if}">
						<div colspan="{$col_span_subtotal}" class="text-right">{l s='Total shipping'}</div>
						<div colspan="2" class="price" id="total_shipping" >{displayPrice price=$total_shipping_tax_exc}</div>
					</div>
                {/if}
            {/if}
			<div class="cart_total_voucher{if $total_discounts == 0} unvisible{/if}">
				<div colspan="{$col_span_subtotal}" class="text-right">
				...
				<div colspan="2" class="price-discount price" id="total_discount">
                    {if $use_taxes && $priceDisplay == 0}
            ...
            {if $use_taxes && $show_taxes && $total_tax != 0 }
                {if $priceDisplay != 0}
					<div class="cart_total_price">
						<div colspan="{$col_span_subtotal}" class="text-right">{if $display_tax_label}{l s='Total (tax excl.)'}{else}{l s='Total'}{/if}</div>
						<div colspan="2" class="price" id="total_price_without_tax">{displayPrice price=$total_price_without_tax}</div>
					</div>
                {/if}
				<div class="cart_total_tax">
					<div colspan="{$col_span_subtotal}" class="text-right">{l s='Tax'}</div>
					<div colspan="2" class="price" id="total_tax">{displayPrice price=$total_tax}</div>
				</div>
            {/if}
			<div class="cart_total_price">
				<div colspan="{$col_span_subtotal}" class="total_price_container text-right">
					<span>{l s='Total'}</span>
                ...
                {if $use_taxes}
					<div colspan="2" class="price" id="total_price_container">
