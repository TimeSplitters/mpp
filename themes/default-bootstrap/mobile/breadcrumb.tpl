...
<div class="breadcrumb clearfix">
	...
	{if isset($path) AND $path}
		...
		{if $path|strpos:'span' !== false}
			<span class="navigation_page">{$path|@replace:'<a ': '<span itemscope itemtype="http://data-vocabulary.org/Breadcrumb"><a itemprop="url" '|@replace:'data-gg="">': '><span itemprop="title">'|@replace:'</a>': '</span></a></span>'}<span id="breadcrumb_product_main_caracteristics"></span></span>
		{else}
			{$path}
		{/if}
	{/if}
